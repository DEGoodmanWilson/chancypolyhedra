...
namespace chancypolyhedra_
{
    ...
    struct operators
    {
        operators()
        {
            ...
            insert( "**", order( 1 ), []( const long l, const long r ) { return std::pow(l,r); } );
            insert( "d", order( 3 ), []( const long l, const long r ) {
                ...
                std::uniform_int_distribution<> dis(1, r_);
                long accum{0};

                for(unsigned long i = 0; i < l_; ++i)
                {
                    accum += die_sign*dis(rng);
                }

                return mult_sign*accum;
            } );
    ...
    struct number
            : seq< opt< one<'d'>>, opt< one< '+', '-' > >, plus< digit > >
    {
    };
    ...
    struct action< number >
    {
        ...
        static void apply( const Input& in, const operators&, stacks& s )
        {
            ...
            {
                //strip initial 'd'
                auto d = std::stol( in.string().erase(0,1) );
                ...
                randutils::auto_seed_128 seeds;
                std::mt19937 rng{seeds};
                std::uniform_int_distribution<> dis(1, d);
                s.push(die_sign*dis(rng));
            }
            else
